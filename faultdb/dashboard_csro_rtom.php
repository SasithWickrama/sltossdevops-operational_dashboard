  ...
  <nav class="navbar fixed-top navbar-expand-lg navbar-light white scrolling-navbar" style="background-image: linear-gradient(to right,#f4ecf7,#d4e6f1);">
    <div class="container-fluid">
      ...
      <a class="navbar-brand waves-effect" style="margin-left:5%">
        <strong class="blue-text" >RTSU - Operational Management Dashboard- (OPMC WG only)&nbsp;<?php echo $username; ?></strong>
  ...
  <div class="container-fluid mt-5">
    ...
    <div class="row wow fadeIn">
      ...
      <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item" style="width: 50%;">
          <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true">CSRO Level-  Pending Faults Distribution</a>
        </li>
        <li class="nav-item" style="width: 50%;">
          <a class="nav-link" id="profile-tab" data-toggle="tab" href="#profile" role="tab" aria-controls="profile" aria-selected="false">Region/Province & RTOM Level-  Pending  Faults Distribution</a>
        </li>
      </ul>
    ...
    <!--Grid column-->
            <div class="col-md-6 mb-4">
              ...
              <div class="card" >
              <div class="card-header text-center">
                  Service Type Level- Faults Count - Region
                  <input type="text"  style="display: none;" name="csro" id="csro" value="<?php echo  htmlentities($username);?>" >
                </div>
                <!--Card content-->
                <div class="card-body" id="chrtDiv3" style="height:380px;overflow:auto"></div>
                 <!--  <canvas id=""></canvas> -->
    ...
    <!--Grid column-->
            <div class="col-md-6 mb-4">
              ...
              <div class="card-header text-center">
                ...
                <!--Card content-->
                <!--<div class="card-body" id="chrtDiv" style="height:380px;overflow:auto"></div>-->
				 <!--  <canvas id=""></canvas> -->
				 <div class="card-body"  style="height:380px;">
                    <div id="rtomfault" style="height:310px; overflow:auto"></div>   
				</div>
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=pendingbox1",
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=pendingboxpri",
   success: function(res){
  if(res != ''){
    console.log(res);
    var elements = document.getElementsByClassName('a');
  ...
  $.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=repfsummary",
   success: function(res){
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=faultnew",
   success: function(res){
  if(res != ''){
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=rtomfault",
   success: function(res){
  if(res != ''){
    
  $("#rtomfault").css("display", "block");
  $("#rtomfault").html();
  $("#rtomfault").html(res["reply"]);
    
    $("#serchD").on("keyup", function() {
    var value = $(this).val().toLowerCase();
    $("#rDatatbl1 tr").filter(function() {
      $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
    });
    });
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=totcount",
   success: function(res){
  if(res != ''){
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=repetedfaults2",
   success: function(res){
  if(res != ''){
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=rtomtot",
   success: function(res){
  ...
  for(var i=0; i<resArr.length; i++){
    ...
    data: {
      ...
      {
        //label: lblArr,
        backgroundColor: ["#6241f5","#8e5ea2","#1f9704","#cf2d21","#6241f5","#8e5ea2","#1f9704","#c45850","#04e3b7","#e5d508","#0bdbf0","#e924c5","#f97f05","#03d7d4","#cf2d21","#cf2d21","#6241f5","#1f9704","#8e5ea2","#1f9704","#c45850","#04e3b7","#c45850","#0bdbf0"],
        data: dataArr
      }
      ]
    },
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=provincetot",
   success: function(res){
  ...
  for(var i=0; i<resArr.length; i++){
    ...
    data: {
      ...
      {
        //label: lblArr,
        backgroundColor: ["#6241f5","#8e5ea2","#1f9704","#c45850","#04e3b7","#e5d508","#0bdbf0","#e924c5","#f97f05","#03d7d4","#cf2d21"],
        data: dataArr
      }
      ]
    },
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=regiontot",
   success: function(res){
  ...
  for(var i=0; i<resArr.length; i++){
    ...
    new Chart(document.getElementById("regiontot"), {
    type: 'horizontalBar',
    data: {
      labels: lblArr,
      datasets: [
      {
        //label: lblArr,
        backgroundColor: ["#6241f5","#8e5ea2","#1f9704","#d44904","#fa1105 "],
        data: dataArr
      }
      ]
    },
...
//function createCharts(){
  ...
  //var csro = document.getElementById('csro').value;
  //$('#chrtDiv').html('');
       
  //$.ajax({
      //type: "POST",
     // data: {csro:csro},
      //url: "./function_C.php?q=1",
  ...
  //$.ajax({
     ...
     // var resArr = resdata.split('@')
      ...
      //for(var i=1; i<resArr.length; i++){
       ...
       // $.ajax({
           //type: "POST",
          // data: {csro:csro,rtom:rtom},
          // url: "./function_C.php?q=3",
           //success: function(res){
             
             // numrec++;
             
            //loadchart(res,csro,numrec,rtom);
           //}
        //});
...
//function loadchart(res,csro,id,rtom){
...
//}
 ...
 //console.log(dataArr)
    ...
    //type: 'horizontalBar',
    //data: {
      //labels: ["Megaline","Broadband", "PEO-TV","FTTH","LTE","DATA","Total"],
     // datasets: [
       // {
          //label: "'fault count",
          //backgroundColor: ["#3e95cd", "#8e5ea2","#3cba9f","#e8c3b9","#f5b041","#97a7e7","#f12110"],
          //data: dataArr
        //}
     // ]
   // },
...
function getChartData2(){
  ...
  $.ajax({
      ...
      for(var i=1; i<resArr.length; i++){
        ...
        $.ajax({
           type: "POST",
           data: {csro:csro,province:province},
           url: "./function_C.php?q=6",
           success: function(res){
             
              numrec++;
             
            loadchart2(res,csro,numrec,province);
...
function loadchart2(res,csro,id,province){
...
function createCharts3(){
  ...
  $.ajax({
      type: "POST",
      data: {csro:csro},
      url: "./function_C.php?q=7",
      success: function(recCount){
  

      for(var n=1; n<=recCount; n++){
        
        $('#chrtDiv3').append('<div style="text-align:center; font-weight:bold; font-size:15px;" id="chatregion'+n+'" class="mb-3" ></div>'+
                  '<div id="chartcontainer3'+n+'" >'+
                  '<canvas id="chartCanves3'+n+'" style="width:200px;">'+'</canvas></div><br/><br/>');
      }
...
function getChartData3(){
  ...
  $.ajax({
      ...
      for(var i=1; i<resArr.length; i++){
        ...
        $('#chatregion'+i).html(region);
        
        $.ajax({
           type: "POST",
           data: {csro:csro,region:region},
           url: "./function_C.php?q=9",
           success: function(res){
             
              numrec++;
             
            loadchart3(res,csro,numrec,region);
...
function loadchart3(res,csro,id,region){
...
for(var i=0; i<resArr.length; i++){
  ...
  dataArr.push(resArr[i]);
    ...
    window.mybarChart3 = new Chart(document.getElementById("chartCanves3"+id), {
    type: 'horizontalBar',
    data: {
      labels: ["Megaline","Broadband", "PEO-TV","FTTH","LTE","DATA"],
      datasets: [
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=repfaults",
   success: function(res){
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=clerdfaults",
   success: function(res){
...
$.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=pendingflts",
   success: function(res){
...
function viewData(){
  ...
  var id = event.target.id.replace("D", "");  
   $.ajax({
     type: "POST",
     url: "./api_C_rtom.php?apicall=repetedfaultsdetails",
     data:{id:id},
     success: function(res){
 ...
 function viewData1(){
  ...
  var id = event.target.id.replace("D", "");  
   $.ajax({
     type: "POST",
     url: "./api_C_rtom.php?apicall=repetedfaultsdetails2",
     data:{id:id},
     success: function(res){
 ...
 $.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=boxfaultsdetails",
 ...
 $.ajax({
   type: "POST",
   url: "./api_C_rtom.php?apicall=boxfaultsdetailspri",
   data:{type:type,time:time},
   success: function(res){
  if(res != ''){
...
var htmls = "";
            ...
            var link = document.createElement("a");
            link.download = "LEA level-Faults Counts-CSRO.xls";
            link.href = uri + base64(format(template, ctx));
            link.click();
}
